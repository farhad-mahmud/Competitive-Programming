...
class MD5 {
    ...
    void transform(const uint8_t block[64]) {
        ...
        HH(a, b, c, d, x[9], 4, 0xd9d4d039);   HH(d, a, b, c, x[12], 11, 0xe6bd9991); HH(c, d, a, b, x[15], 16, 0x1fa27cf8); HH(b, c, d, a, x[2], 23, 0xc4ac5665);
...
public:
    ...
    std::string finalize() {
        uint8_t bits[8];
        for (int i = 0; i < 8; i++) bits[i] = (uint8_t)((((uint64_t)count[i >> 2] << 3) >> ((i & 3) * 8)) & 0xFF);
        ...
        uint8_t digest[16];
        for (int i = 0; i < 16; i++) digest[i] = (uint8_t)((state[i >> 2] >> ((i & 3) * 8)) & 0xFF);
    ...
    static std::string hash(const std::string &str) {
        MD5 md5;
        md5.update((const uint8_t*)str.c_str(), str.length());
        return md5.finalize();
    }
};
...
int main() {
    ...
    std::cout << MD5::hash(input) << std::endl;
    return 0;
}